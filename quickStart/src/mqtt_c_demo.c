	...
	else
	{
		...
		{                                     
		             ...
		             //memcpy(imageName,object->valueint,strlen(object->valuestring));
                             sprintf(payload,"{\"services\":[{\"service_id\":\"testID\",\"properties\":{\"imageName\":\"%s\"},\"eventTime\":null}]}",name->valuestring);
		...
		{                                     
                             char payload[200];
		             //memcpy(imageName,object->valueint,strlen(object->valuestring));
                             sprintf(payload,"{\"services\":[{\"service_id\":\"testID\",\"properties\":{\"class\":\"%s\"},\"eventTime\":null}]}",class->valuestring);
                             char *report_topic = combine_strings(3, "$oc/devices/", username, "/sys/properties/report");
		             ret = mqtt_publish(report_topic, payload);
		             free(report_topic);
		             report_topic = NULL;				                          
